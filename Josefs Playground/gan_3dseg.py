...
def discriminator(x_image, reuse=False):
	with tf.name_scope("discriminator"):
		...
		d_w1 = tf.get_variable("d_w1", [20, 20, 1, 32], initializer=tf.truncated_normal_initializer(stddev=0.02))
		...
		d_w2 = tf.get_variable("d_w2", [20, 20, 32, 64], initializer=tf.truncated_normal_initializer(stddev=0.02))
...
def generator(batch_size, z_dim):
	with tf.name_scope("gernerator"):
		...
		g_w1 = tf.get_variable("g_w1", [z_dim, 10 * 20 * 20 * 50], dtype=tf.float32, initializer=tf.truncated_normal_initializer(stddev=0.02))
		g_b1 = tf.get_variable("g_b1", [10 * 20 * 20 * 50], initializer=tf.truncated_normal_initializer(stddev=0.02))
		#print(g_w1.shape)
...
def gen_plot(image):
	plt.figure()
	plt.imshow(image[:, :, 0], cmap='Greys', interpolation='nearest')
...
def train():
	...
	sum_d_r_loss = tf.summary.scalar('Discriminator_loss_real', d_loss_real)
	sum_d_f_loss = tf.summary.scalar('Discriminator_loss_fake', d_loss_fake)
	...
	plot_buf = gen_plot(np.zeros([100, 100, 1]))
	...
	with sess.as_default():
		...
		for i in range(300):
			...
			_, __, ___, ____ = sess.run([d_trainer_real, d_trainer_fake],
										{x_placeholder: batch})
		...
		for i in np.arange(EPOCH-300) + 300:

			batch = image_batch.eval()

			#  _, dLossReal, dLossFake, gLoss = sess.run([d_trainer_real, d_loss_real, d_loss_fake, g_loss], {x_placeholder: batch})
			if dLossFake > 0.6:
			...
			if i % 10 == 0:
				...
				with sess.as_default():
					batch = image_batch.eval()
					#plot_buf = gen_plot(batch[0])

				#image = tf.image.decode_png(plot_buf.getvalue(), channels=4)
				#image = tf.expand_dims(image, 0)
				#input_img = tf.summary.image("plot", image)
				#merged = tf.summary.merge([sum_g_loss, sum_d_f_loss, sum_d_r_loss, input_img])
